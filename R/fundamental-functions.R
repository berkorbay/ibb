...
#'@param resource_id Resource ID of the wanted data set
...
get_data_frame_ibb <- function(resource_id,limit="",return_json=FALSE){
  ...
  return_df <- json_query_response$result$records %>% rename_all(~gsub("^_","",.)) %>% rename_all(~gsub(" |\\(|\\)|/|-","_",.)) %>% rename_all(~gsub("^_|_+$","",.)) %>% tbl_df()
